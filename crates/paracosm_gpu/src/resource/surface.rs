use crate::{device::Device, resource::{SyncLabel, SyncLabelComponent}};
...
pub trait SurfaceLabel: SyncLabel { }

#[derive(Component, Default)]
pub struct PrimarySurface;
impl SurfaceLabel for PrimarySurface { }
impl SyncLabel for PrimarySurface { }
...
pub(crate) struct Surface {
    ...
    pub api: ash::khr::surface::Instance,
...
impl Surface {
    pub fn configure(&mut self, device: &Device, config: SurfaceConfig) -> Result<()> {
        ...
        let surface_formats = unsafe { self.api.get_physical_device_surface_formats(*device.physical_device, self.surface)? };
        ...
        let surface_capabilities = unsafe {
            self.api.get_physical_device_surface_capabilities(*device.physical_device, self.surface)?
        };
        ...
        unsafe { device.swapchain.destroy_swapchain(self.swapchain, None); }
        ...
        self.device = device.logical_device.swapchain.clone();
    ...
    pub fn acquire(&mut self) -> Result<(ash::vk::ImageMemoryBarrier2<'static>, ash::vk::Semaphore, ash::vk::Semaphore)> {
        let acquire_semaphore = self.acquire_semaphores[self.frame_index as usize];
        let (image_index, suboptimal) = unsafe { self.device.acquire_next_image(self.swapchain, u64::MAX, acquire_semaphore, ash::vk::Fence::null())? };
    ...
    pub fn finish(&mut self) -> Result<ash::vk::ImageMemoryBarrier2<'static>> {
        let barrier = ash::vk::ImageMemoryBarrier2::default()
            .src_stage_mask(ash::vk::PipelineStageFlags2::ALL_COMMANDS)
            .src_access_mask(ash::vk::AccessFlags2::MEMORY_WRITE)
            .dst_stage_mask(ash::vk::PipelineStageFlags2::ALL_COMMANDS)
    ...
    pub fn present(&mut self, queue: ash::vk::Queue) -> Result<()> {
        ...
        let suboptimal = unsafe { self.device.queue_present(queue, &present_info)? };
...
impl crate::context::Context {
    pub fn create_surface<L: SurfaceLabel + 'static>(
        ...
        let mut surface = Surface {
            device: device.logical_device.swapchain.clone(),
            api, 
            surface,
        ...
        let format = surface.configure(&device, config)?;
